	...
	const getUserProfileJSX = () => {
		if (responseHeaders) {
			if (isLoggedIn) {
                    ...
                    */
					<div className="d-flex flex-row justify-content-center align-items-center">
						<div className="d-flex flex-column justify-content-center align-items-center">
							<Image
								src={responseHeaders.get("x-mk8-pretendo-imageurl") as string}
								width={48}
								height={48}
				...
				return (
					<Link
						href={`https://pretendo.network/account/login?redirect=${window.location.origin + pathname}`}
						className="active nav-link me-5"
					>
						<strong className="me-2 text-primary">
							Login
							<MdLogin size={25} className="ms-2" />
						</strong>
					</Link>
				);
			}
						...
						<div className="d-flex gap-2 align-items-center ms-3">
							...
							<div className="d-flex flex-column">
								<span className="text-muted fs-6">
									<strong>Mario Kart 8 Pretendo website</strong>
								</span>
							</div>
						</div>
					</Link>
				...
				<Navbar.Collapse className="justify-content-between me-3 ms-2">
					<Nav className="ms-3">
						...
						<NavigationBarEntry
							path="/gatherings"
							name="Gatherings"
							desc="List of matchmaking sessions (worldwide, regional, friend rooms, tournaments)"
						/>
						...
						<NavigationBarEntry path="/bans" name="Bans" desc="List of bans" />
					</Nav>
